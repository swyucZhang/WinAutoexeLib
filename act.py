...
class key(act):
    def __init__(self,value):
        self.value = value
    def hold(self):
        win32api.keybd_event(VK_CODE[self.value],0,0,0)
    def press(self):
        win32api.keybd_event(VK_CODE[self.value],0,0,0)
        win32api.keybd_event(VK_CODE[self.value],0,win32con.KEYEVENTF_KEYUP,0)
    def release(self):
        win32api.keybd_event(VK_CODE[self.value],0,win32con.KEYEVENTF_KEYUP,0)
...
class mouse(act):
    ...
    def doubleClick(self,x = None,y = None):
        ...
        win32api.mouse_event(win32con.MOUSEEVENTF_LEFTDOWN,0,0,0,0)
        win32api.mouse_event(win32con.MOUSEEVENTF_LEFTUP,0,0,0,0)
    def click_r(self,x = None,y = None):
        if not x is None and not y is None:
            win32api.SetCursorPos((x,y))
            time.sleep(0.02)
        win32api.mouse_event(win32con.MOUSEEVENTF_RIGHTDOWN,0,0,0,0)
        win32api.mouse_event(win32con.MOUSEEVENTF_RIGHTUP,0,0,0,0)
    def doubleClick_r(self,x = None,y = None):
        if not x is None and not y is None:
            win32api.SetCursorPos((x,y))
            time.sleep(0.02)
        win32api.mouse_event(win32con.MOUSEEVENTF_RIGHTDOWN,0,0,0,0)
